...
where
    ...
    K: AsfaloadPublicKeyTrait<Signature = S>,
    <K as signatures::keys::AsfaloadPublicKeyTrait>::VerifyError: std::fmt::Display,
{
    ...
    let is_valid_signer = if let Some(admin_keys) = &config.admin_keys {
        admin_keys.iter().any(|group| {
            group
                .signers
                .iter()
                .any(|signer| signer.data.pubkey.to_base64() == pubkey.to_base64())
        })
    } else {
        config.artifact_signers.iter().any(|group| {
            group
                .signers
                .iter()
                .any(|signer| signer.data.pubkey.to_base64() == pubkey.to_base64())
