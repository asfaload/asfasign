...
use signatures::keys::{AsfaloadKeyPairTrait, AsfaloadPublicKeyTrait, AsfaloadSecretKeyTrait};
...
fn basic_flow() -> Result<()> {
    ...
    // User1 initialises the signers file
    {
        ...
        //
        // This is the signers file that will be initialised
        let signers_content = r#"
{
  "version": 1,
  "initial_version": {
  ...
  "artifact_signers": [
    {
      "signers": [
        { "kind": "key", "data": { "format": "minisign", "pubkey": "USER1_PUBKEY" } },
        { "kind": "key", "data": { "format": "minisign", "pubkey": "USER2_PUBKEY" } }
      ],
      "threshold": 2
    }
  ],
...
"#;
        ...
        let signers_content = signers_content
            .replace("USER1_PUBKEY", &user1_keypair.public_key().to_base64())
            .replace("USER2_PUBKEY", &user2_keypair.public_key().to_base64());
    ...
    // It reads the content of the signers file proposed by user1
    {
        ...
        sign_signers_file(signers_file, &signature2, &user2_keypair.public_key())?;
    }
    ...
    // User2 signs the text file
    {
        ...
        let signature2 = user2_keypair
            .secret_key("password")?
            .sign(&text_file_hash)?;
        load_for_file::<AsfaloadPublicKey<_>, AsfaloadSignature<_>, _>(&text_file)?
            .get_pending()
            .expect("Expected pending agg sig here")
            .add_individual_signature(&signature2, &user2_keypair.public_key())?;
    }
    ...
    // User1 signs the text file
    {
        ...
        load_for_file::<AsfaloadPublicKey<_>, AsfaloadSignature<_>, _>(&text_file)?
            .get_pending()
            .expect("Expected pending agg sig here")
            .add_individual_signature(&signature1, &user1_keypair.public_key())?;
    }
