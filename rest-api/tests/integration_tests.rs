...
pub mod tests {
    ...
    fn parse_log_lines(content: &str) -> Result<Vec<Value>> {
        content
            .lines()
    ...
    async fn test_request_id_logging_flow() -> Result<()> {
        ...
        // FIXME: sleeping to let server write logs
        // Without this sleep, the test fails because the prased logs
        // don't have entries with the request_id.
        tokio::time::sleep(Duration::from_millis(500)).await;
